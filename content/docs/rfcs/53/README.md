...
slug: 53
title: 53/LZ4TCP
name: ZMTP LZ4+TCP Compressed Transport
status: Draft
editor: Patrik Wenger <paddor@gmail.com>
---

This document specifies `lz4+tcp://`, an LZ4-compressed variant of the ZMTP 3.1 transport.
...
### 2.2 Why not negotiate compression
...
### 2.3 Why LZ4 block format, not frame
...
### 5.2 Uncompressed part (`00 00 00 00`)
...
### 5.3 LZ4-compressed single-block part (`LZ4B`)
...
- `decompressed_size` MUST NOT exceed `LZ4M_BLOCK_SIZE`
  ...
  multi-block encoding (Sec. 5.4).

### 5.4 LZ4-compressed multi-block part (`LZ4M`)
...
2. `00 00 00 00`: check remaining byte count against the budget
   (Sec. 7); return the remaining `N - 4` bytes as plaintext.
3. `LZ4B`:
...
### 9.1 CRIME / BREACH-style compression oracles
...
allocate a huge output buffer. The per-part size-budget check (Sec. 7)
...
`LZ4M_BLOCK_SIZE` (1 GiB) by the receiver (Sec. 5.7 rule 3). Parts
